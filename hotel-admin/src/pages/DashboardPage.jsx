...
export default function DashboardPage() {
  ...
  const overviewCards = data ? [
    { icon: '💰', label: 'Revenus', value: fmtMoney(data.summary.totalRevenue), color: '#15803d', bg: '#f0fdf4' },
    { icon: '💸', label: 'Dépenses', value: fmtMoney(data.summary.totalExpenses), color: '#b45309', bg: '#fff7ed' },
    { icon: '📈', label: 'Bénéfice net', value: fmtMoney(data.summary.netProfit), color: data.summary.netProfit >= 0 ? '#2563eb' : '#dc2626', bg: '#eff6ff' },
    ...
    ...(isOwner && realEstate ? [
      { icon: '🏘️', label: 'Baux actifs', value: fmtCount(realEstate.rentStatus.activeLeases), color: '#1d4ed8', bg: '#eff6ff' },
      { icon: '🧾', label: 'Loyers impayés', value: fmtMoney(realEstate.rentStatus.outstanding), color: '#b91c1c', bg: '#fef2f2' },
      { icon: '📊', label: 'Taux recouvrement', value: fmtPercent(realEstate.rentStatus.collectionRate), color: '#166534', bg: '#f0fdf4' },
    ] : []),
  ] : [];
  ...
  const executiveStats = data ? [
    { label: 'Taux chambres', value: `${data.rooms.occupancyRate}%`, tone: getOccupancyColor(data.rooms.occupancyRate), sub: `${fmtCount(data.rooms.occupied)} / ${fmtCount(data.rooms.total)}` },
    { label: 'Taux salles', value: `${data.halls.occupancyRate}%`, tone: getOccupancyColor(data.halls.occupancyRate), sub: `${fmtCount(data.halls.booked)} / ${fmtCount(data.halls.total)}` },
    ...
    ...(isOwner && realEstate ? [
      { label: 'Recouvrement loyers', value: fmtPercent(realEstate.rentStatus.collectionRate), tone: realEstate.rentStatus.collectionRate >= 80 ? '#16a34a' : '#dc2626', sub: `${fmtMoney(realEstate.rentStatus.collected)} / ${fmtMoney(realEstate.rentStatus.expected)}` },
    ] : []),
  ...
  return (
    <AdminLayout title={isOwner ? 'Dashboard superviseur' : 'Tableau de bord'}>
      <div style={s.page}>
        <section style={s.welcome}>
          <div style={s.heroMain}>
            <div style={s.rolePill}>{isOwner ? 'Vue propriétaire / superviseur' : 'Vue opérationnelle'}</div>
            <h2 style={s.welcomeTitle}>Bonjour, {userName || 'Utilisateur'} 👋</h2>
        ...
        {data && (
          <section style={s.executiveCard}>
            <div style={s.executiveTop}>
              <div>
                <h3 style={s.sectionTitle}>Synthèse exécutive</h3>
                <p style={s.sectionText}>Vue rapide de la santé opérationnelle et financière de l’hôtel.</p>
              </div>
              <div style={s.periodPillWrap}>
                <span style={s.periodPill}>{appliedFilters.mode === 'month' ? 'Filtre mensuel' : 'Période personnalisée'}</span>
        ...
        <section style={s.filtersCard}>
          <div style={s.filtersTop}>
            <div>
              ...
              <p style={s.sectionText}>Analyse les activités de l’hôtel sur un mois précis ou sur une période personnalisée.</p>
            </div>
        ...
        {data && (
          <>
            {quickActions.length > 0 && (
              <section style={s.quickActionsCard}>
                <div style={s.quickActionsHead}>
                  <div>
                    <h3 style={s.sectionTitle}>Actions rapides</h3>
                    <p style={s.sectionText}>Accès direct aux écrans clés de supervision.</p>
                  </div>
                </div>
                <div style={s.quickActionsGrid}>
                  {quickActions.map((action) => (
                    <button key={action.path} type="button" style={s.quickActionBtn} onClick={() => navigate(action.path)}>
                      ...
                      <span style={s.quickActionArrow}>→</span>
                    </button>
                  ))}
                </div>
              </section>
            ...
            <section style={s.grid3}>
              <Panel title="Répartition des revenus" subtitle="Sources de revenus de la période">
                ...
                <MiniStat label="Appartements & loyers" value={fmtMoney(data.revenue.apartment)} color="#2563eb" />
                <MiniStat label="Encaissement du jour" value={fmtMoney(data.revenue.today)} color="#b45309" />
              </Panel>

              <Panel title="Répartition des dépenses" subtitle="Charges enregistrées sur la période">
              ...
              <Panel title="Activité de réservation" subtitle="Vue synthétique des opérations">
            ...
            {isOwner && realEstate && (
              <>
                <section style={s.grid2}>
                  <Panel title="Suivi des loyers" subtitle="Vue propriétaire sur les loyers dus, encaissés et impayés">
...
function StatCard({ icon, label, value, color, bg, onClick }) {
  ...
  return (
    <button
      ...
      style={{ ...s.statCard, background: bg, cursor: onClick ? 'pointer' : 'default', transform: hovered && onClick ? 'translateY(-2px)' : 'none', opacity: onClick ? 1 : 0.98 }}
    >
...
const s = {
  ...
  progressCard: { background: 'rgba(255,255,255,0.12)', backdropFilter: 'blur(8px)', padding: '16px', borderRadius: '14px' },
  ...
  executiveItem: { border: '1px solid #e2e8f0', borderRadius: '14px', padding: '16px', background: 'linear-gradient(180deg, #ffffff 0%, #f8fafc 100%)' },
  ...
  quickActionBtn: { border: '1px solid #e2e8f0', background: 'linear-gradient(180deg, #ffffff 0%, #f8fafc 100%)', borderRadius: '14px', padding: '16px', display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: '6px', cursor: 'pointer', textAlign: 'left' },
  ...
  statCard: { border: 'none', borderRadius: '16px', padding: '18px 18px', display: 'flex', gap: '14px', alignItems: 'center', textAlign: 'left', boxShadow: '0 2px 10px rgba(15,23,42,0.06)', transition: 'transform 0.16s ease' },
  ...
  grid2: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '16px' },
  panel: { background: '#fff', borderRadius: '16px', padding: '22px 24px', boxShadow: '0 2px 10px rgba(15,23,42,0.06)' },
